...
class Edrone():
	...
	#	8. Add error_sum
		...
		if modulus(self.error[0]) <0.5 and modulus(self.error[1]) <0.5 and modulus(self.error[2]) <1 and self.i<35:
			#self.end_time = time.time()
			#self.continous = self.start_time - self.end_time
			i#f(modulus(self.cummulative_time) >=10):
			self.i += 1
			#	self.cummulative_time = 0			
			#self.cummulative_time += self.continous
			
		elif modulus(self.error[0]) <0.5 and modulus(self.error[1]) <0.5 and modulus(self.error[2]) <1 and self.i==35:	
			self.arm()
			while(True):
				self.disarm() 
				print("disarming")
		...
		print("for cell "+str(list[self.i])+" the cummulative time is "+ str(round(self.cummulative_time,1)) +" continous time is "+str(round(self.continous,1)))
