...
* Functions: 
	main functions -> constraint(), modulus() 
	class edrone functions ->  __init__(), disarm(), arm(), whycon_callback(), altitude_set_pid(), pitch_set_pid(), roll_set_pid(), decision_callback(), land() ,pid()
* Global Variables: 
	list, waypoints, data
'''
...
'''
* Function Name: contraint()
* Input:	value -> input which is to be contrained within minimum and maximum limits
 	 	 	limit_max -> maximum limit upto which 
			limit_min

* Output:	value in range from limit_min to limit_max
* Logic:	if value > limit_max then value = limit_max
			else if value <limit_min then value = limit_min
			else return value only
*
* Example Call: constraint(3,1,9)
'''
...
* Input:	number
* Output:	positive value of number 
* Logic:	if value > 0 then return value
			else value = -value
...
class Edrone():
	...
	def __init__(self):
		...
		rospy.Subscriber('/serviced_info',SRInfo,self.land)
	...
	* Output:	return the index in list defined globally for input msg.location 
	* Logic:	list.index(msg.location) return index of list for msg.location 

	*
	* Example Call: self.decision_callback(msg)
	'''
	
	def decision_callback(self,msg):
		self.i = list.index(msg.location)

	#----------------------------------------------------------------------------------------------------------------------
	'''
	* Function Name: land()
	* Input:	msg (type SRInfo from survey_and_rescue.msg)
	* Output:	return the index of base in list 
	* Logic:	if msg.info is equal to 'END' then return index 27 i.e for BASE 

	*
	* Example Call: self.land(msg)
	'''
	...
	#-----------------------------------------------------------------------------------------------------------------------
		...
		self.command_pub.publish(self.cmd)
		print("current location is " + list[self.i])
		#print(self.i)
...
if __name__ == '__main__':
	...
	data = {"A1": [-8.02, -8.378, 26.614], "A3": [-8.421, -1.772, 29.698], "A2": [-8.224, -5.145, 28.277], "A5": [-7.67, 5.03, 28.098], "A4": [-8.157, 1.777, 29.162], "A6": [-7.459, 8.093, 26.033], "C3": [-1.642, -1.961, 30.602], "E4": [5.11, 1.639, 29.991], "C1": [-1.436, -8.566, 27.852], "E6": [4.958, 7.977, 26.707], "E1": [4.795, -8.743, 27.369], "C6": [-1.401, 8.205, 27.754], "E3": [5.106, -2.1, 29.827], "C4": [-1.612, 1.397, 30.811], "D5": [1.876, 4.812, 29.205], "E5": [5.145, 4.763, 28.856], "C2": [-1.645, -5.12, 29.342], "F1": [7.909, -8.718, 26.413], "F2": [8.32, -5.327, 27.317], "F3": [8.403, -1.993, 28.743], "F4": [8.419, 1.577, 28.575], "F5": [8.403, 4.679, 27.45], "F6": [7.914, 7.755, 25.384], "B4": [-4.971, 1.69, 30.678], "B5": [-4.434, 5.133, 28.836], "B6": [-4.558, 8.245, 27.356], "C5": [-1.263, 4.974, 29.568], "B1": [-4.803, -8.657, 27.651], "B2": [-5.116, -5.172, 29.374], "B3": [-4.949, -1.98, 30.165], "D6": [1.956, 8.055, 27.437], "D4": [2.038, 1.428, 30.509], "E2": [5.009, -5.433, 28.981], "D2": [1.677, -5.329, 29.494], "D3": [1.765, -2.016, 30.442], "D1": [1.728, -8.662, 27.713]}
	for x in list:
